...
async def _call_ai_provider(system_prompt: str, user_message: str) -> str:
    ...
    raise HTTPException(
        ...
        detail="BurkinaSira AI n'est pas encore configuré (clé API du fournisseur LLM manquante).",
    )
...
async def send_message(data: SendMessageRequest, user_id: str) -> ConversationResponse:
    ...
    ai_reply_text = await _call_ai_provider(
        system_prompt=f"BurkinaSira AI — assistant touristique Burkina Faso, type: {data.type.value}",
